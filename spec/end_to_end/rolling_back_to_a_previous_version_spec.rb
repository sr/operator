...
describe "rollback back to a previous version" do
  ...
  before do
    stub_request(:get, "http://canoe.test/api/targets/test/deploys/latest?repo_name=pardot&server=#{ShellHelper.hostname}")
      .to_return(body: %({"id":445,"what":"branch","what_details":"master","artifact_url":"#{first_artifact_url}","build_number":#{first_build_number},"created_at":"2015-12-20T14:26:29-05:00", "servers":{"#{ShellHelper.hostname}":{"stage":"pending","action":"deploy"}}}))

    bootstrap_repo_path(tempdir)

    first_version = BuildVersion.new(first_build_number, first_sha, first_artifact_url)
    ...
    current_version = BuildVersion.new(current_build_number, current_sha, current_artifact_url)

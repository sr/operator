  ...
  RSpec.describe Hostname do
    describe "#shard" do
      it "returns db for DAL and SEA shard servers" do
        expect(Hostname.new("db-d1").shard.prefix).to eq("db")
        expect(Hostname.new("db-s1").shard.prefix).to eq("db")
      end

      it "returns whoisdb for whois servers" do
        expect(Hostname.new("whoisdb-d1").shard.prefix).to eq("whoisdb")
        expect(Hostname.new("whoisdb-s1").shard.prefix).to eq("whoisdb")
        expect(Hostname.new("pardot0-whoisdb1-1-dfw").shard.prefix).to eq("whoisdb")
      end

      it "returns db for DFW and PHX shard servers" do
        expect(Hostname.new("pardot0-dbshard1-88-dfw").shard.prefix).to eq("db")
        expect(Hostname.new("pardot0-dbshard1-88-phx").shard.prefix).to eq("db")
      end

      it "parses DAL and SEA servers" do
        expect(Hostname.new("db-d1").shard.id).to eq(1)
        expect(Hostname.new("db-s2").shard.id).to eq(2)
        expect(Hostname.new("whoisdb-d1").shard.id).to eq(1)
        expect(Hostname.new("whoisdb-s2").shard.id).to eq(2)
      end

      it "parses DFW and PHX servers" do
        expect(Hostname.new("pardot0-dbshard1-88-dfw").shard.id).to eq(88)
        expect(Hostname.new("pardot0-dbshard1-89-phx").shard.id).to eq(89)
        expect(Hostname.new("pardot0-whoisdb1-1-dfw").shard.id).to eq(1)
        expect(Hostname.new("pardot0-whoisdb1-1-phx").shard.id).to eq(1)
      end
    end

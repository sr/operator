workspace(name = "com_github_sr_operator")

git_repository(
    name = "io_bazel_rules_go",
    commit = "878185a8d66cb4a3fb93602d8e8bc2f50cd69616",
    remote = "https://github.com/bazelbuild/rules_go.git",
)

git_repository(
    name = "com_github_google_protobuf",
    commit = "cd315dcbadc02569e145bde16e3f66c2fbb08e31",
    remote = "https://github.com/google/protobuf",
)

load("@io_bazel_rules_go//go:def.bzl", "go_repositories", "go_repository", "new_go_repository")

new_go_repository(
	name = "com_github_golang_lint",
	commit = "206c0f020eba0f7fbcfbc467a5eb808037df2ed6",
	importpath = "github.com/golang/lint",
)

go_repositories()

go_repository(
    name = "com_github_bazelbuild_buildifier",
    commit = "31c314a5d60d94f6c67de61e9228d1aeb1f6c8e5",
    importpath = "github.com/bazelbuild/buildifier",
)

new_go_repository(
    name = "org_golang_x_tools",
    commit = "8b84dae17391c154ca50b0162662aa1fc9ff84c2",
    importpath = "golang.org/x/tools",
)

new_go_repository(
    name = "com_github_acsellers_inflections",
    commit = "cb98bfe9e3ee28e8649778f5fbb2e95f54f71b39",
    importpath = "github.com/acsellers/inflections",
)

new_go_repository(
    name = "com_github_golang_protobuf",
    commit = "8e1c791771cf5854f2402ad63f68b05672e099d2",
    importpath = "github.com/golang/protobuf",
)

new_go_repository(
    name = "com_github_kr_text",
    commit = "7cafcd837844e784b526369c9bce262804aebc60",
    importpath = "github.com/kr/text",
)

new_go_repository(
    name = "com_github_sr_operator_generator",
    commit = "2c881b18d426a78f969559d037bc2e442578a7e3",
    importpath = "github.com/sr/operator/generator",
)

new_go_repository(
    name = "org_golang_x_net",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net",
)

new_go_repository(
    name = "org_golang_x_net_http2",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/http2",
)

new_go_repository(
    name = "org_golang_x_net_http2_hpack",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/http2/hpack",
)

new_go_repository(
    name = "org_golang_x_net_internal_timeseries",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/internal/timeseries",
)

new_go_repository(
    name = "org_golang_x_net_lex_httplex",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/lex/httplex",
)

new_go_repository(
    name = "org_golang_x_net_trace",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/trace",
)

new_go_repository(
    name = "org_golang_google_grpc",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc",
)

new_go_repository(
    name = "org_golang_google_grpc_codes",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/codes",
)

new_go_repository(
    name = "org_golang_google_grpc_credentials",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/credentials",
)

new_go_repository(
    name = "org_golang_google_grpc_grpclog",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/grpclog",
)

new_go_repository(
    name = "org_golang_google_grpc_internal",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/internal",
)

new_go_repository(
    name = "org_golang_google_grpc_metadata",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/metadata",
)

new_go_repository(
    name = "org_golang_google_grpc_naming",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/naming",
)

new_go_repository(
    name = "org_golang_google_grpc_peer",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/peer",
)

new_go_repository(
    name = "org_golang_google_grpc_transport",
    commit = "941cc894cea3c87a12943fd12b594964541b6d28",
    importpath = "google.golang.org/grpc/transport",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_aes",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/aes",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_arrays",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/arrays",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_base64url",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/base64url",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_compact",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/compact",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_kdf",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/kdf",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_keys_ecc",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/keys/ecc",
)

new_go_repository(
    name = "com_github_dvsekhvalnov_jose2go_padding",
    commit = "2a04a552f4a8adf7d51731877b7a2021bab968bb",
    importpath = "github.com/dvsekhvalnov/jose2go/padding",
)

new_go_repository(
    name = "com_github_sr_operator",
    commit = "2c881b18d426a78f969559d037bc2e442578a7e3",
    importpath = "github.com/sr/operator",
)

new_go_repository(
    name = "org_golang_x_net_context_ctxhttp",
    commit = "4971afdc2f162e82d185353533d3cf16188a9f4e",
    importpath = "golang.org/x/net/context/ctxhttp",
)

new_go_repository(
    name = "org_golang_x_oauth2",
    commit = "d5040cddfc0da40b408c9a1da4728662435176a9",
    importpath = "golang.org/x/oauth2",
)

new_go_repository(
    name = "org_golang_x_oauth2_clientcredentials",
    commit = "d5040cddfc0da40b408c9a1da4728662435176a9",
    importpath = "golang.org/x/oauth2/clientcredentials",
)

new_go_repository(
    name = "org_golang_x_oauth2_internal",
    commit = "d5040cddfc0da40b408c9a1da4728662435176a9",
    importpath = "golang.org/x/oauth2/internal",
)

new_go_repository(
    name = "com_github_matttproud_golang_protobuf_extensions",
    commit = "c12348ce28de40eed0136aa2b644d0ee0650e56c",
    importpath = "github.com/matttproud/golang_protobuf_extensions",
)

new_go_repository(
    name = "com_github_satori_go_uuid",
    commit = "b061729afc07e77a8aa4fad0a2fd840958f1942a",
    importpath = "github.com/satori/go.uuid",
)

new_go_repository(
    name = "com_github_sr_operator_protolog_pb",
    commit = "2c881b18d426a78f969559d037bc2e442578a7e3",
    importpath = "github.com/sr/operator/protolog/pb",
)

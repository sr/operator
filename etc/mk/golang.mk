...
PACKAGES ?= $(shell $(GO) list bread/... privet/... devenv/... github.com/sr/operator/... | grep -Ev '^devenv/vendor/|bread/deploy/vendor')
...
deadleaves: $(DEADLEAVES)
	@ out="$$($< 2>&1 | grep -Ev '(github.com/hashicorp/terraform|^devenv/vendor/|^bread/deploy/vendor|^github.com/sr/operator/testing$')')"; \
		if [ -n "$$out" ]; then \
			echo "$$out"; \
			exit 1; \
		fi

...
func TestForkHandler(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "PR with the same head user as base user. Should not be closed.",
			pullRequest: &github.PullRequest{
				Org:        "pardot",
				Repository: "bread",
				Number:     1,
		...
		{
			name: "PR with a different head user as base user. Should be closed.",
			pullRequest: &github.PullRequest{
				Org:        "pardot",
				Repository: "bread",
				Number:     1,
			...
			expectedReplyComment: &issueReplyCommentMatcher{
				...
				BodyRegexps: []*regexp.Regexp{
					regexp.MustCompile(`@user.*pull requests originating from forked repositories are not mergeable`),

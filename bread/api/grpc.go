...
// ChatCommand is a gRPC call sent as a chat message.
...
type CommandInvoker func(context.Context, *grpc.ClientConn, *ChatCommand) error

// GRPCMessageHandler returns a MessageHandler that detects commands in chat
// messages and turns them into gRPC calls.
func GRPCMessageHandler(defaultPackage string, c chan<- *ChatCommand) ChatMessageHandler {
...
func HandleChatCommand(messenger Messenger, invoker CommandInvoker, timeout time.Duration, conn *grpc.ClientConn, cmd *ChatCommand) error {
	if messenger == nil {
	...
	if err != nil && !strings.Contains(err.Error(), "unhandleable command:") && !strings.Contains(err.Error(), "unknown service") {
		_ = messenger(context.TODO(), &ChatMessage{
...
func newMetadata(cmd *ChatCommand) metadata.MD {
...
func findCommand(msg string) *ChatCommand {

...
class UserQueryTest < ActiveSupport::TestCase
  ...
  test "execute" do
    ...
    results = query.execute(@user, @session)
    row = results.first
  ...
  test "execute account query from pardot-full account" do
    ...
    query = @user.account_query("SELECT * FROM object_audit", 1)
    results = query.execute(@user, my_session)
    row = results.first
    assert_equal 1, row[:id]
  end
  ...
  test "global tables" do
    ...
    tables = query.database_tables(@session)
  ...
  test "rate limiting" do
    begin
      ...
      @user.global_query("SELECT 1").execute(@user, @session)
      @user.global_query("SELECT 1").execute(@user, @session)

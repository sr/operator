module PullAgent
  ...
  class Storm
    ...
    def add_topology(topo, topo_env, jar)
      add_topo_command = [STORM_BIN, "jar", jar, "com.pardot.storm.topology.TopologyRunner", "--topo-def=#{topo_class(topo)}", "--name=#{topo_name(topo)}", "--remote", "--config-file=config/topologies/#{topo_name(topo)}.yaml,config/environments/#{topo_env}.yaml,/etc/storm/environment.yaml"]

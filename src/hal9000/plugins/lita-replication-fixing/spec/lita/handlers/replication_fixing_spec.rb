...
describe Lita::Handlers::ReplicationFixing, lita_handler: true do
  ...
  describe "POST /replication/errors" do
    it "attempts to fix the error and notifies the ops room" do
      stub_request(:get, "https://repfix-dfw.pardot.com/replication/" /
        "fixes/for/db/1/1")
        .and_return(
      ...
      fix_request = stub_request(:post, "https://repfix-dfw.pardot.com/" /
        "replication/fix/db/1")
      ...
      response = http.post("/replication/errors",
                           URI.encode_www_form(
                             ...
                             "mysql_last_error" => "Query: 'INSERT INTO " /
                             "foo VALUES ('foo@example.com', '1', '1.2')"
    ...
    it "notifies the ops-replication room with a sanitized error messages" do
      ...
      expect(
        replies[0]
      ).to eq("pardot0-dbshard1-1-dfw: Replication is broken")
      expect(
        replies[1]
      ).to eq("pardot0-dbshard1-1-dfw: Query: 'INSERT INTO foo VALUES " /
      "([REDACTED], '1', '1.2')")
    end
  ...
  describe "!ignore" do
    ...
    it "ignores the shard with a given prefix for 15 minutes by default" do
      ...
      expect(
        replies.last
      ).to eq("OK, I will ignore whoisdb-1-dfw for 15 minutes")
    end
    ...
    it "allows the number of minutes to be specified" do
      ...
      expect(
        replies.last
      ).to eq("OK, I will ignore whoisdb-1-dfw for 10 minutes")
    end
  end
  ...
  describe "!fix" do
    it "attempts to fix the shard" do
      stub_request(:get,
                   "https://repfix-dfw.pardot.com/replication/fixes/for/db/11")
        .and_return(
          { body: JSON.dump("is_erroring" => true, "is_fixable" => true) },
          body: JSON.dump("is_erroring" => true, "is_fixable" => true,
                          "fix" => { "active" => true })
      ...
      request = stub_request(:post,
                             "https://repfix-dfw.pardot.com/replication" /
                             "/fix/db/11")
                .and_return(body: JSON.dump("is_erroring" => true,
                                            "is_fixable" => true))
    ...
    it "attempts to fix the shard in PHX" do
      stub_request(:get,
                   "https://repfix-phx.pardot.com/replication/fixes/for/db/11")
        .and_return(
          { body: JSON.dump("is_erroring" => true, "is_fixable" => true) },
          body: JSON.dump("is_erroring" => true, "is_fixable" => true,
                          "fix" => { "active" => true })
      ...
      request = stub_request(:post,
                             "https://repfix-phx.pardot.com/replication/" /
                             "fix/db/11")
                .and_return(body: JSON.dump("is_erroring" => true,
                                            "is_fixable" => true))
  ...
  describe "!currentautofixes" do
    it "lists the fixes currently ongoing" do
      fixing_status_client = ::ReplicationFixing::FixingStatusClient.new(
        "dfw",
        subject.redis
      )
      fixing_status_client.set_active(shard: ::ReplicationFixing::Shard.new(
        "db", 12, "dfw"
      ), active: true)
      fixing_status_client.set_active(shard: ::ReplicationFixing::Shard.new(
        "db", 32, "dfw"
  ...
  describe "!stopfixing and !startfixing" do
    it "globally stops and starts fixing" do
      ...
      expect(
        replies.last
      ).to eq("OK, I've stopped fixing replication for ALL shards")

      send_command("checkfixing")
      expect(
        replies.last
      ).to eq("(nope) Replication fixing is globally disabled in dfw")

      send_command("startfixing")
      expect(
        replies.last
      ).to eq("OK, I've started fixing replication")

      send_command("checkfixing")
      expect(
        replies.last
      ).to eq("(goodnews) Replication fixing is globally enabled in dfw")
    end
    ...
    it "globally stops and starts fixing in PHX" do
      ...
      expect(
        replies.last
      ).to eq("(nope) Replication fixing is globally disabled in phx")
      ...
      expect(
        replies.last
      ).to eq("(goodnews) Replication fixing is globally enabled in phx")
    end
  end
  ...
  describe "!status" do
    it "prints the status of each database host" do
      ...
      stub_request(:get,
                   "https://repfix-dfw.pardot.com/replication/fixes/for/db/1")
        .and_return(body: JSON.dump(response))

      send_command("status 1")
      expect(
        replies.last
      ).to eq("status for db-1-dfw\n* dfw: 0 seconds behind\n* dfw2: " /
      "2 seconds behind")
    end
  end
end

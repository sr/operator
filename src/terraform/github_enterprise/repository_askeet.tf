...
resource "github_team_repository" "askeet_site-reliability-engineers" {
  repository = "${github_repository.askeet.name}"
  team_id    = "${github_team.site-reliability-engineers.id}"
  permission = "admin"
}

resource "github_team_repository" "askeet_engineering-managers" {
  repository = "${github_repository.askeet.name}"
  team_id    = "${github_team.engineering-managers.id}"
  permission = "admin"
}

resource "github_team_repository" "askeet_service-accounts-write-only" {
  repository = "${github_repository.askeet.name}"
  team_id    = "${github_team.service-accounts-write-only.id}"
  permission = "push"
}

resource "github_team_repository" "askeet_service-accounts-admins" {
  repository = "${github_repository.askeet.name}"

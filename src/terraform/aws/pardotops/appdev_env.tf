...
resource "aws_instance" "appdev_containerlab1" {
  key_name = "internal_apps"
  count = "${var.environment_appdev["num_containerlab1_hosts"]}"
  ...
  tags {
    Name = "${var.environment_appdev["pardot_env_id"]}-containerlab1-${count.index + 1}-${var.environment_appdev["dc_id"]}"
    terraform = "true"
  }
}

resource "aws_route53_record" "appdev_containerlab1_arecord" {
  count = "${var.environment_appdev["num_containerlab1_hosts"]}"
  zone_id = "${aws_route53_zone.appdev_aws_pardot_com_hosted_zone.zone_id}"
  name = "${var.environment_appdev["pardot_env_id"]}-containerlab1-${count.index + 1}-${var.environment_appdev["dc_id"]}.${aws_route53_zone.appdev_aws_pardot_com_hosted_zone.name}"
  records = ["${element(aws_instance.appdev_containerlab1.*.private_ip, count.index)}"]
  type = "A"
  ttl = "900"
}
